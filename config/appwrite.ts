...
export const config = {
  ...
  APPWRITE_PAIRING_COLLECTION_ID:
    process.env.NEXT_PUBLIC_APPWRITE_PAIRING_COLLECTION_ID,
};
...
if (
  ...
  !config.APPWRITE_PROJECT ||
  !config.APPWRITE_DATABASE_ID ||
  !config.APPWRITE_COLLECTION_ID
) {
  throw new Error(
    "Appwrite endpoint, project, database ID, and collection ID must be defined",
  );
}
...
export const initiatePairing = async (partnerEmail: string) => {
  try {
    ...
    if (partnerDocumentResponse.documents.length > 0) {
      return {
        success: false,
        reason: "Partner is already paired with someone else.",
      };
    }
    ...
    if (userDocumentResponse.documents.length > 0) {
      return {
        success: false,
        reason: "You are already paired with someone else.",
      };
    }
    ...
    return {
      success: false,
      reason: "An error occurred while initiating pairing.",
    };
  }
};
...
export const confirmPairing = async (partnerEmail: string) => {
  try {
    ...
    const partnerDocumentResponse = await database.listDocuments(
      ...
      config.APPWRITE_PAIRING_COLLECTION_ID as string,
      [
        Query.equal("userEmail", partnerEmail),
        Query.equal("partnerEmail", email),
      ],
    );
